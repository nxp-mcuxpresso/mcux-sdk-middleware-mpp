...
{
    // Detect if we are inside an ISR
    ctx->in_isr = xPortIsInsideInterrupt();

    if (ctx->in_isr) {
        // ISR version — return old BASEPRI/state
        ctx->key = taskENTER_CRITICAL_FROM_ISR();
    } else {
        // Task version — does not return a mask
        taskENTER_CRITICAL();
        ctx->key = 0;   // will not be used
    }
    ctx->active = true;
}
...
{
    assert(ctx != NULL && ctx->active);

    ctx->active = false;

    if (ctx->in_isr) {
        taskEXIT_CRITICAL_FROM_ISR(ctx->key);
    } else {
        taskEXIT_CRITICAL();
    }
}
