...
/* @brief This example application tests a graph with two Splits branches:
 * 2D camera -> split -> image converter -> draw labeled rectangle -> display
 *                   +-> image converter -> split -> null sink
 *                                               +-> TensorFlow Lite model MobileNet v1
 * The camera view finder is displayed on screen.
 * Only the ML inference element is executed in a task with flag MPP_EXEC_PREEMPT.
...
int mpp_event_listener(mpp_t mpp, mpp_evt_t evt, void *evt_data, void *user_data) {
    ...
    case MPP_EVENT_INFERENCE_OUTPUT_READY:
        ...
        /* check that we can modify user data (not accessed by other task) */
...
static void app_task(void *params)
{
    ...
    mpp_t mp_split_rc;
    ret = mpp_split(mp, 1 , &mpp_params, &mp_split_rc);
    if (ret) {
    ...
    ret = mpp_element_add(mp_split_rc, MPP_ELEMENT_CONVERT, &elem_params, NULL);
    if (ret ) {
        PRINTF("Failed to add element CONVERT\r\n");
        goto err;
    }

    /* split again the pipeline into 2 branches, new branch is preemptable */
    mpp_t mp_split_pr;
    mpp_params.exec_flag = MPP_EXEC_PREEMPT;
    ret = mpp_split(mp_split_rc, 1 , &mpp_params, &mp_split_pr);
    if (ret) {
        PRINTF("Failed to split pipeline again\r\n");
        goto err;
    }

    /* close the pipeline with a null sink */
    ret = mpp_nullsink_add(mp_split_rc);
    if (ret) {
    ...
    ret = mpp_element_add(mp_split_pr, MPP_ELEMENT_INFERENCE, &mobilenet_params, NULL);
    if (ret) {
        PRINTF("Failed to add element VALGO_TFLite\r\n");
        goto err;
    }
    /* close the pipeline with a null sink */
    ret = mpp_nullsink_add(mp_split_pr);
    if (ret) {
        PRINTF("Failed to add NULL sink\r\n");
        goto err;
    }
