...
#include "couple_COCO_320_240_jpeg.h"
#include "tiger_320_240_jpeg.h"
...
#define CHANGE_FRAME_DATA  (0)           /* Enable frame data switching between different image sources */
#define FRAME_CHANGE_MS  (1000U)         /* Interval for switching between images */
...
const uint8_t *rgb_image_data = &couple_COCO_320_240_jpeg_data[0];
uint32_t rgb_image_size = sizeof(couple_COCO_320_240_jpeg_data);
const uint8_t *ir_image_data = &tiger_320_240_jpeg_data[0];
uint32_t ir_image_size = sizeof(tiger_320_240_jpeg_data);
...
#if CHANGE_FRAME_DATA
    uint32_t  image_change_start = 0;
#endif
    uint32_t time;
...
#if CHANGE_FRAME_DATA
        /*** Frame rate tracking and image data cycling logic ***/
        time = xTaskGetTickCount();
        if (((time - image_change_start) * portTICK_PERIOD_MS) > FRAME_CHANGE_MS)
        {
#if (FRAME_CHANGE_MS >= 2000)
            PRINTF("Change frame data after %d ms\r\n", (time - image_change_start) * portTICK_PERIOD_MS);
#endif
            crt_rgb_image_data = crt_rgb_image_data == rgb_image_data ? ir_image_data : rgb_image_data;
            crt_rgb_image_size = crt_rgb_image_size == rgb_image_size ? ir_image_size : rgb_image_size;
            crt_ir_image_data = crt_ir_image_data == ir_image_data ? rgb_image_data : ir_image_data;
            crt_ir_image_size = crt_ir_image_size == ir_image_size ? rgb_image_size : ir_image_size;

            image_change_start = xTaskGetTickCount();
        }
#endif
    }
