 ...
 * The model performs multiple face detection
 ...
 * The detection zone width and height depend on the display_aspeect_ratio compared to the model aspect_ratio:
...
#define TC_NAME "test_camera_ultraface_view_config" CONFIG2STR(APP_CONFIG)
#else
#define TC_NAME "test_camera_ultraface_view"
#endif
...
int main()
{
    ...
    PRINTF("****** %s TEST test_camera_ultraface_view ******\r\n", TEST_MODE);
 ...
 *  0: cannot state
 */
static int check_ultraface_inference_output(box_data final_boxes[], int num_detections)
{
    int status = 0;
#if (SOURCE_STATIC_IMAGE == 1)
    if (num_detections != EXPECTED_NUM_DETECTED_FACES) {
    	PRINTF("\r\nERROR: expected to detect %d face(s) but got %d face(s) detected\r\n", EXPECTED_NUM_DETECTED_FACES, num_detections);
    	status = -1;
    } else {
...
int mpp_event_listener(mpp_t mpp, mpp_evt_t evt, void *evt_data, void *user_data) {
    ...
    case MPP_EVENT_INFERENCE_OUTPUT_READY:
        ...
        infer_check = check_ultraface_inference_output(app_priv->boxes, app_priv->detected_count);
        if (chksm_done)
        {
        	PRINTF("\r\nStart %s\r\n", TC_NAME);
...
static void app_task(void *params)
{
    ...
    /* On the preempt-able branch run the ML Inference (using an ultraface-slim TF-Lite model) */
